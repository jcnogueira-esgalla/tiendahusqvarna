...
<div class="w-100 mt-4">
	...
	<a href="<?php echo get_permalink( $producto->get_id() ) ?>" class="ficha-accesorio d-flex flex-column justify-content-between shadow">
		...
		<?php if($producto->get_stock_status()=='instock' && !in_array(wp_get_post_terms( $producto->id, 'product_cat' )[0]->term_id, [1815, 479])): ?>
			<button href="<?php echo get_site_url().'/?add-to-cart='.$producto->get_id() ?>" class="btn btn-primary btn-cart text-white spacing font-weight-regular border-0 fs-125 text-center text-uppercase boton-add-cart-ficha" data-quantity="1" data-product_id="<?php echo $producto->get_id() ?>"><?php _e('AÑADIR AL CARRITO', 'esgalla') ?></button>
		<?php else: ?>
			<button href="<?php echo get_permalink( $producto->get_id() ) ?>" class="btn btn-primary btn-cart text-white spacing font-weight-regular border-0 fs-125 text-center text-uppercase" ><?php _e('VER PRODUCTO', 'esgalla') ?></button>
		<?php endif; ?>
